...
public class TestBDD {
    ...
    public void testOperations() {
        ...
        assertEquals("BDD canonicity (and/or/not)", n1, n2);
        ...
        assertEquals("BDD canonicity (biimp)", b1, b2);
        ...
        assertEquals("NAND consistency", na1, na2);
        jdd.deref(na1);
        jdd.deref(na2);
        jdd.deref(naeq);
        ...
        assertEquals("NOR consistency", no2, no1);
        jdd.deref(no1);
        jdd.deref(no2);
        jdd.deref(noeq);
        ...
        assertEquals("Exist failed (1)", jdd.exists(toq, cube), v1);
        assertEquals("Exist failed (2)", jdd.exists(nv1, cube), nv1);
        assertEquals("Forall failed (1)", 0, jdd.forall(toq, cube));
        assertEquals("Forall failed (2)", jdd.forall(qor, v2), v1);
        assertEquals("Forall failed (3)", jdd.forall(qor, v1), v2);
        assertEquals("Forall failed (4)", jdd.forall(qor, v3), qor);
        ...
        assertEquals("replace() test (1)", p1, v3v4);
        assertEquals("replace() test (2)", p2, v1v2);
        assertEquals("replace() test (3)", p3, v2v4);
        assertEquals("replace() test (4)", p4, v1v3);
    ...
    public void testTemp() {
        ...
        jdd.nstack.pop();
        jdd.gc();
        assertFalse("previously intermediate node garbage collected", jdd.isValid(tmp));
    }
    ...
    public void testMember() {
        ...
        assertFalse("member (1)", jdd.member(p1, mb));
        assertTrue("member (2)", jdd.member(p2, mb));
