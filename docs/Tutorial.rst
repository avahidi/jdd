...
To see if everything works correctly, enter the JDD directory and execute the following commands::
...
This part explains the basic BDD operations. It assumes, however, that you are familiar with BDDs and related data structures.

**Creating a BDD manager**

The first thing to do is to create a BDD manager, which will hold all your variables and BDDs.
You may have several BDD managers in the same application. But you cannot directly exchange information between them.
...
To do anything interesting you first need some BDD variables. In JDD, BDD variables are represented by Java integers::
...
As garbage collection can happen at any time, it is important that BDDs are referenced as soon as possible.
...
It is sometimes desired to substitute variables in a tree. To do this, you first need to formally define your permutation::
...
Now we have two permutations: one that maps v1 to v2 and another that maps v2 to v1. We apply them using the replace() function::
...
ZDDGraph is intended to include common ZDD operations used in graph algorithms, as explained in Coudert's paper::
...
Internally, BDDPrinter.printDot() is used to generated DOT representations and images from BDDs.
